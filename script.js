        ...
        if (response.ok) {
            ...
            if (result.valid) {
                ...
                if (result.newExpiresAt) {
                    currentUser.tokenExpiresAt = result.newExpiresAt;
...
function initTokenCheck() {
    ...
    if (currentUser && currentUser.token) {
        ...
        // 每分钟检查一次令牌状态
        tokenCheckInterval = setInterval(checkTokenStatus, 60000);
    }
}
        ...
        if (response.ok) {
            ...
            if (result.valid) {
                ...
                currentUser = {
                    ...
                    tokenExpiresAt: result.tokenExpiresAt // 新增：保存令牌过期时间
                };
                localStorage.setItem('currentUser', JSON.stringify(currentUser));
