...
async function handleVisitsRequest(request, kv) {
  if (request.method === 'GET') {
    ...
    try {
      ...
      // 查找现有记录
      const existingVisit = visits.find(v => v.ip === clientIP);
    ...
    if (request.method === 'GET') {
      ...
      if (auth.user.isAdmin && !auth.user.isSuperAdmin) {
        ...
        if (targetUser && (targetUser.isAdmin || targetUser.isSuperAdmin)) {
          return new Response(JSON.stringify({ error: '无权编辑管理员账户' }), {
            status: 403,
            headers: {
              'Content-Type': 'application/json',
              'Access-Control-Allow-Origin': '*',
      ...
      if (updateData.approved !== undefined) {
        users[userIndex].approved = updateData.approved;
      }
      if (updateData.frozen !== undefined) {
        users[userIndex].frozen = updateData.frozen;
      }
      if (updateData.securityQuestion !== undefined) {
        users[userIndex].securityQuestion = updateData.securityQuestion;
      }
      if (updateData.securityAnswer !== undefined) {
        users[userIndex].securityAnswer = simpleEncrypt(updateData.securityAnswer);
      }
      if (updateData.permissions !== undefined) {
        users[userIndex].permissions = updateData.permissions;
      }
      
      // 只有超级管理员可以设置角色
      if (auth.user.isSuperAdmin) {
        if (updateData.isSuperAdmin !== undefined) {
          users[userIndex].isSuperAdmin = updateData.isSuperAdmin;
        }
        if (updateData.isAdmin !== undefined) {
          users[userIndex].isAdmin = updateData.isAdmin;
        }
      }
      
      await kv.put('users', JSON.stringify(users));
      
      // 如果用户被冻结或权限变更，撤销其所有令牌
      if (updateData.frozen || updateData.permissions !== undefined || 
          updateData.isAdmin !== undefined || updateData.isSuperAdmin !== undefined) {
        const tokenManager = new TokenManager(kv);
      ...
      if (auth.user.isAdmin && !auth.user.isSuperAdmin) {
        ...
        if (targetUser && (targetUser.isAdmin || targetUser.isSuperAdmin)) {
          return new Response(JSON.stringify({ error: '无权删除管理员账户' }), {
            status: 403,
            headers: {
              'Content-Type': 'application/json',
              'Access-Control-Allow-Origin': '*',
  ...
  if (request.method === 'POST') {
    try {
      ...
      const newUser = {
        ...
        securityAnswer: securityAnswer ? simpleEncrypt(securityAnswer) : '',
        permissions: {
          viewHangarTimes: false
        },
        createdAt: new Date().toISOString()
      };
...
async function handleRecoverPasswordRequest(request, kv) {
  ...
  if (request.method === 'POST') {
    try {
      ...
      if (user && simpleDecrypt(user.securityAnswer) === securityAnswer) {
        // 返回密码
        return new Response(JSON.stringify({
          success: true,
...
async function handleResetRequest(request, kv) {
  if (request.method === 'POST') {
    try {
      ...
      // 重置所有数据（保留管理员账户）
      const users = await kv.get('users', { type: 'json' }) || [];
      const adminUsers = users.filter(u => u.isAdmin || u.isSuperAdmin);
      
      await kv.put('config', JSON.stringify({}));
      await kv.put('users', JSON.stringify(adminUsers));
      await kv.put('feedback', JSON.stringify([]));
      await kv.put('visits', JSON.stringify([]));
