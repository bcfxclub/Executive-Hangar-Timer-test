        ...
        if (response.ok) {
            ...
            feedback.forEach(item => {
                ...
                feedbackItem.innerHTML = `
                    <div class="feedback-date">${formatDateTimeFull(new Date(item.timestamp))}</div>
                    <div class="feedback-username">${item.username || '匿名用户'}</div>
                    <div class="feedback-content">${item.content}</div>
                    ${item.contact ? `<div class="feedback-contact">联系方式: ${item.contact}</div>` : ''}
        ...
        if (response.ok) {
            ...
            visits.forEach(visit => {
                ...
                visitItem.innerHTML = `
                    ...
                    <div class="visit-date">${formatDateTimeFull(new Date(visit.timestamp))}</div>
                `;
    ...
    try {
        ...
        if (response.ok) {
            ...
            users.forEach(user => {
                ...
                const roleBadge = user.isSuperAdmin ? '超级管理员' : 
                                user.isAdmin ? '管理员' : 
                                user.approved ? '普通用户' : '待审核';
                const roleClass = user.isSuperAdmin ? 'user-super-admin' : 
                                user.isAdmin ? 'user-admin' : 
                                user.approved ? 'user-normal' : 'user-pending';
                const frozenClass = user.frozen ? 'user-frozen' : '';
                
                userItem.innerHTML = `
                    <div class="user-info">
                        <div><strong>${user.username}</strong> <span class="user-role-badge ${roleClass} ${frozenClass}">${roleBadge}${user.frozen ? ' (冻结)' : ''}</span></div>
                        <div style="font-size: 0.8rem; color: #666;">${user.email || '无邮箱'}</div>
                        <div style="font-size: 0.8rem; color: #666;">注册时间: ${formatDateTimeFull(new Date(user.registrationTime))}</div>
                    </div>
                    <div class="user-actions">
                        <button class="user-action-btn edit-user" data-id="${user.id}">编辑</button>
                        ${!user.isSuperAdmin ? `<button class="user-action-btn delete-user" data-id="${user.id}">删除</button>` : ''}
                    </div>
                `;
...
function showUserEditModal(user) {
    ...
    document.getElementById('edit-role').value = user.isSuperAdmin ? 'super-admin' : user.isAdmin ? 'admin' : 'normal';
    ...
    // 更新权限复选框
    updatePermissionCheckboxes(user);
    
    document.getElementById('user-edit-modal').style.display = 'flex';
}

// 更新权限复选框
function updatePermissionCheckboxes(user) {
    // 隐藏所有管理员权限复选框
    document.querySelectorAll('.admin-permission').forEach(el => {
    ...
    document.querySelectorAll('#edit-permissions input[type="checkbox"]').forEach(checkbox => {
        ...
        checkbox.checked = permissions[permissionName] || false;
    });
}
...
document.getElementById('save-user-edit').addEventListener('click', async function() {
    ...
    // 收集权限
    document.querySelectorAll('#edit-permissions input[type="checkbox"]').forEach(checkbox => {
        userData.permissions[checkbox.value] = checkbox.checked;
    });
...
function loadUserInfo() {
    if (currentUser) {
        ...
        const roleText = currentUser.isSuperAdmin ? '超级管理员' : 
                        currentUser.isAdmin ? '管理员' : '普通用户';
        document.getElementById('user-info-role').textContent = roleText;
        
        // 加载密保问题
        if (currentUser.securityQuestion) {
            document.getElementById('user-security-question').value = currentUser.securityQuestion;
        }
    }
}
...
document.getElementById('update-security-question').addEventListener('click', async function() {
    ...
    try {
        const response = await fetch(`${API_BASE}/update-security`, {
            method: 'POST',
            headers: getAuthHeaders(),
            body: JSON.stringify({ 
                securityQuestion, 
...
document.getElementById('project-description-btn').addEventListener('click', function() {
    ...
    document.getElementById('footer-modal-content').innerHTML = document.getElementById('project-description-input').value.replace(/\n/g, '<br>');
...
document.getElementById('version-btn').addEventListener('click', function() {
    ...
    document.getElementById('footer-modal-content').innerHTML = document.getElementById('version-input').value.replace(/\n/g, '<br>');
    document.getElementById('footer-info-modal').style.display = 'flex';
});

document.getElementById('about-btn').addEventListener('click', function() {
    document.getElementById('footer-modal-title').textContent = '关于我们';
    document.getElementById('footer-modal-content').innerHTML = document.getElementById('about-input').value.replace(/\n/g, '<br>');
    document.getElementById('footer-info-modal').style.display = 'flex';
});
...
function initializeTimer(initialPhase) {
    ...
    // 调整开始时间
    const offsetMinutes = config.offset;
...
function updateLights() {
    ...
    currentLights.forEach((light, index) => {
        const lightElement = document.createElement('div');
        lightElement.className = `light ${light} active`;
        lightElement.id = `light-${index}`;
...
function updatePhaseIndicator() {
    ...
    switch (currentPhase) {
        case 'reset':
            phaseIndicator.className = 'phase-indicator phase-reset';
            phaseIndicator.innerHTML = '<i class="fas fa-sync-alt"></i> <span>机库已关闭等待开启中</span>';
            break;
        case 'card':
            phaseIndicator.className = 'phase-indicator phase-card';
            phaseIndicator.innerHTML = '<i class="fas fa-id-card"></i> <span>机库大门已开启可以插卡</span>';
            break;
        case 'poweroff':
            phaseIndicator.className = 'phase-indicator phase-poweroff';
            phaseIndicator.innerHTML = '<i class="fas fa-power-off"></i> <span>机库关闭中请耐心等待</span>';
            break;
        default:
            phaseIndicator.className = 'phase-indicator phase-disabled';
            phaseIndicator.innerHTML = '<i class="fas fa-exclamation-triangle"></i> <span>计时器维护中</span>';
    }
}
...
function updateTimer() {
    if (!timerEnabled) {
        document.getElementById('countdown').textContent = '维护中';
        document.getElementById('phase-indicator').className = 'phase-indicator phase-disabled';
        document.getElementById('phase-indicator').innerHTML = '<i class="fas fa-exclamation-triangle"></i> <span>计时器维护中</span>';
        document.getElementById('hangar-open-time').textContent = '当前机库开启时间: 维护中';
        return;
    }
    
    const now = new Date();
    const elapsed = Math.floor((now - adjustedStartTime) / 1000); // 秒
    const totalCycleTime = (PHASE_DURATIONS.reset + PHASE_DURATIONS.card + PHASE_DURATIONS.poweroff) * 60; // 秒
    ...
    if (cycleTime < PHASE_DURATIONS.reset * 60) {
        // 重置阶段
        currentPhase = 'reset';
        remainingTime = PHASE_DURATIONS.reset * 60 - cycleTime;
        nextPhase = 'card';
        
        // 计算重置阶段内的灯光变化
        const resetPhaseTime = cycleTime;
        const lightChangeInterval = (PHASE_DURATIONS.reset * 60) / 5; // 每24分钟变一个灯
        
        currentLights = Array(5).fill('red');
        const lightsToTurnGreen = Math.min(5, Math.floor(resetPhaseTime / lightChangeInterval));
        for (let i = 0; i < lightsToTurnGreen; i++) {
            currentLights[i] = 'green';
        }
    } else if (cycleTime < (PHASE_DURATIONS.reset + PHASE_DURATIONS.card) * 60) {
        // 插卡阶段
        currentPhase = 'card';
        remainingTime = (PHASE_DURATIONS.reset + PHASE_DURATIONS.card) * 60 - cycleTime;
        nextPhase = 'poweroff';
        
        // 计算插卡阶段内的灯光变化
        const cardPhaseTime = cycleTime - PHASE_DURATIONS.reset * 60;
        const lightChangeInterval = (PHASE_DURATIONS.card * 60) / 5; // 每12分钟变一个灯
        
        currentLights = Array(5).fill('green');
        const lightsToTurnGray = Math.min(5, Math.floor(cardPhaseTime / lightChangeInterval));
        for (let i = 0; i < lightsToTurnGray; i++) {
            currentLights[i] = 'gray';
        }
    } else {
        // 关机阶段
        currentPhase = 'poweroff';
        remainingTime = totalCycleTime - cycleTime;
        nextPhase = 'reset';
    ...
    // 更新机库开启时间显示
    const nextPhaseTime = new Date(now.getTime() + remainingTime * 1000);
    document.getElementById('hangar-open-time').textContent = `当前机库开启时间: ${formatDateTime(nextPhaseTime)}`;
}
...
// 计算机库开启时间
function calculateHangarOpenTimes(baseTime) {
    const now = new Date();
    const totalCycleTime = (PHASE_DURATIONS.reset + PHASE_DURATIONS.card + PHASE_DURATIONS.poweroff) * 60 * 1000; // 毫秒
    
    // 计算从基准时间到现在的总毫秒数
    const elapsed = now - baseTime;
    
    // 计算当前周期的开始时间
    const cyclesCompleted = Math.floor(elapsed / totalCycleTime);
    const currentCycleStart = new Date(baseTime.getTime() + cyclesCompleted * totalCycleTime);
    
    // 机库开启时间（插卡阶段的开始时间）
    const hangarOpenOffset = PHASE_DURATIONS.reset * 60 * 1000; // 毫秒
    
    const times = [];
    
    // 上次开启时间
    const lastOpenTime = new Date(currentCycleStart.getTime() + hangarOpenOffset - totalCycleTime);
    times.push(`上次开启时间: ${formatDateTime(lastOpenTime)}`);
    
    // 下次开启时间
    const nextOpenTime = new Date(currentCycleStart.getTime() + hangarOpenOffset);
    times.push(`下次开启时间: ${formatDateTime(nextOpenTime)}`);
    
    // 后续7次开启时间
    for (let i = 1; i <= 7; i++) {
        const futureOpenTime = new Date(nextOpenTime.getTime() + i * totalCycleTime);
        times.push(`开启时间 ${i}: ${formatDateTime(futureOpenTime)}`);
    }
    ...
    const canViewHangarTimes = currentUser && 
        (currentUser.permissions?.viewHangarTimes || currentUser.isAdmin || currentUser.isSuperAdmin);
    
    if (hangarTimesVisible || canViewHangarTimes) {
        // 显示完整的机库开启时间预测
        times.forEach((time, index) => {
            const li = document.createElement('li');
            li.innerHTML = `<i class="fas fa-window-restore"></i> ${time}`;
            windowList.appendChild(li);
        });
    } else {
        // 仅显示基本信息，隐藏具体时间
        const li = document.createElement('li');
        li.innerHTML = `<i class="fas fa-window-restore"></i> 机库开启时间预测仅对注册授权用户开放`;
        windowList.appendChild(li);
    }
}
...
function initDraggablePanel(panelId) {
    ...
    const header = panel.querySelector('.admin-panel-header') || panel.querySelector('.user-center-header');
...
async function loadTokenStats() {
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('token-total').textContent = stats.total || 0;
            document.getElementById('token-active').textContent = stats.active || 0;
            document.getElementById('token-expired').textContent = stats.expired || 0;
            document.getElementById('token-auto-clean').textContent = (stats.autoCleanDays || 30) + '天';
            ...
            if (stats.userStats) {
                ...
                stats.userStats.forEach(userStat => {
                    ...
                    userItem.innerHTML = `
                        ...
                        <span>${userStat.tokenCount} 个令牌</span>
                    `;
                    userList.appendChild(userItem);
                });
            }
...
document.getElementById('set-auto-clean').addEventListener('click', async function() {
    ...
    try {
        const response = await fetch(`${API_BASE}/set-auto-clean`, {
            method: 'POST',
            headers: getAuthHeaders(),
...
document.getElementById('set-token-expiration').addEventListener('click', async function() {
    ...
    try {
        const response = await fetch(`${API_BASE}/set-token-expiration`, {
            method: 'POST',
            headers: getAuthHeaders(),
            body: JSON.stringify({ days })
        });
...
document.getElementById('clean-expired-tokens').addEventListener('click', async function() {
    if (confirm('确定要清理所有过期令牌吗？')) {
        try {
            const response = await fetch(`${API_BASE}/clean-expired-tokens`, {
                method: 'POST',
                headers: getAuthHeaders()
            });
            ...
            if (response.ok) {
                const result = await response.json();
                alert(`已清理 ${result.cleanedCount} 个过期令牌`);
                loadTokenStats(); // 重新加载统计
            } else {
                if (!checkAuthResponse(response)) {
                    return;
...
document.getElementById('clean-all-tokens').addEventListener('click', async function() {
    if (confirm('确定要清理所有令牌吗？所有用户都将需要重新登录！')) {
        try {
            const response = await fetch(`${API_BASE}/clean-all-tokens`, {
                method: 'POST',
                headers: getAuthHeaders()
            });
            ...
            if (response.ok) {
                const result = await response.json();
                alert(`已清理 ${result.cleanedCount} 个令牌`);
                
                // 如果当前用户不是超级管理员，也需要重新登录
                if (!currentUser || !currentUser.isSuperAdmin) {
                    currentUser = null;
                    localStorage.removeItem('currentUser');
                    updateUserInterface();
...
async function recordVisit() {
    try {
        await fetch(`${API_BASE}/record-visit`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            }
...
async function checkDatabaseStatus() {
    try {
        ...
        if (response.ok) {
            document.getElementById('db-status').innerHTML = '<i class="fas fa-database" style="color: #2ecc71;"></i> <span>数据库状态: 连接正常</span>';
        } else {
            document.getElementById('db-status').innerHTML = '<i class="fas fa-database" style="color: #e74c3c;"></i> <span>数据库状态: 连接异常</span>';
        }
    } catch (error) {
        document.getElementById('db-status').innerHTML = '<i class="fas fa-database" style="color: #e74c3c;"></i> <span>数据库状态: 连接失败</span>';
    }
...
// 定期检查数据库状态
setInterval(checkDatabaseStatus, 60000); // 每分钟检查一次
checkDatabaseStatus(); // 立即检查一次
