    ...
    try {
        ...
        if (response.ok) {
            ...
            visits.forEach(visit => {
                ...
                const date = new Date(visit.lastVisit).toLocaleString();
                visitItem.innerHTML = `
            ...
            users.forEach(user => {
                ...
                userItem.innerHTML = `
                    ...
                    <div class="user-actions">
                        ...
                        <button class="user-action-btn delete-user" data-username="${user.username}">删除</button>
                    </div>
                `;
...
document.getElementById('save-user-edit').addEventListener('click', async function() {
    ...
    const updatedUser = {
        ...
        isAdmin: document.getElementById('edit-role').value === 'admin' || document.getElementById('edit-role').value === 'super-admin',
        isSuperAdmin: document.getElementById('edit-role').value === 'super-admin',
    ...
    try {
        const response = await fetch(`${API_BASE}/users/${encodeURIComponent(currentEditingUser.username)}`, {
            method: 'PUT',
            headers: getAuthHeaders(),
        ...
        if (response.ok) {
            ...
            if (currentUser && currentUser.username === currentEditingUser.username) {
                const userResponse = await fetch(`${API_BASE}/users/${encodeURIComponent(currentUser.username)}`, {
                    headers: getAuthHeaders()
                });
                if (userResponse.ok) {
...
document.getElementById('update-user-info').addEventListener('click', async function() {
    ...
    try {
        const response = await fetch(`${API_BASE}/users/${encodeURIComponent(currentUser.username)}`, {
            method: 'PUT',
            headers: getAuthHeaders(),
